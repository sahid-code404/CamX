...
/** Deterministic trust-first ranking for profiles that already passed eligibility. */
internal object LensProfileRanker {
    ...
    private fun sourceRank(target: LensSelectionTarget): Int = when {
        ...
        target.route.physicalCameraId == null && CameraRouteSource.JAVA_DEEP_PROBED in target.route.sources -> 1
        target.route.physicalCameraId != null && CameraRouteSource.JAVA_PHYSICAL in target.route.sources -> 2
        else -> 3
    }
}
